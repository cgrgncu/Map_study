...
%       水平解析度:1-arc-minutes (~1.8 kilometers)，單位[度]，座標系統:WGS84；
...
target_end_lat=21;
    ...
    % 標題
    title({'台灣地區ETOPO1地形圖(含水深)';'水平解析度: 1弧分(約1.85公里)';'垂直解析度: 整數(單位:公尺)'})
    %--
    % 另存成mat檔案
    % 符合DEM的XYZ檔案格式
    % 普通XYZ格式如下:
    ...
    %--
    % 補充資訊
    ETOPO1_Taiwan_DEM.Description='來源:自「https://www.ngdc.noaa.gov/mgg/global/」下載的「ETOPO1_Bed_g_geotiff.tif」中萃取出台灣範圍內的資料。水平解析度:1-arc-minutes (~1.8 kilometers)，單位[度]，座標系統:WGS84；高程解析度:整數，單位[公尺]，高程原點為海平面，向上為正。';
